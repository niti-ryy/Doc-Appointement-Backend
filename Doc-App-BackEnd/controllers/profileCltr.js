...
profileController.create = async (req, res) => {
  ...
  const userData = _.pick(req.body, [
    ...
    "password",
    "role",
  ]); // Pick necessary fields from the request body
  ...
  try {
    ...
    res.status(201).json({
      // Return success message and saved user details
      message: "Registration Successful",
      user: savedUser,
    });
  } catch (error) {
    res.status(500).json({
      // Return error message in case of registration failure
...
profileController.login = async (req, res) => {
  ...
  if (!errors.isEmpty()) {
    return res.status(401).json({
      // Return error message for validation failures during login
  ...
  try {
    ...
    if (!user) {
      return res.status(401).json({
        // Return error message for invalid email during login
    ...
    if (!verifyPassword) {
      return res.status(401).json({
        // Return error message for invalid password during login
        message: "Invalid Email or Password",
      });
    }
    const tokenData = {
    ...
    res.status(201).json({
      // Return success message with generated token for authenticated user
      message: `Bearer ${token}`,
    });
  } catch (e) {
    res.status(401).json({
      // Return error message for any unexpected errors during login
      message: e.message,
    });
  }
};
...
profileController.update = async (req, res) => {
  // Controller function to update a profile based on its ID
  ...
  try {
    const updatedRecord = await Profile.findByIdAndUpdate(id, body, {
      new: true,
    }); // Find profile by ID and update it
    if (!updatedRecord) {
      res.status(401).json({
        // Return error message if no profile found for the provided ID
        message: "No Profile found with the provided ID. Update failed.",
      });
    }
    res.status(200).json({
      // Return success message and updated profile details
      message: updatedRecord,
    });
  } catch (e) {
    res.status(500).json({
      // Return error message for any unexpected errors during update
      message: e.message,
    });
  }
};
...
profileController.getUserAndCounselors = async (req, res) => {
  // Controller function to get users or counselors based on their roles
  ...
  try {
    if (role === "Counselor" || role === "User") {
      ...
      ); // Find profiles based on the specified role
...
profileController.getSingleUserAndCounselor = async (req, res) => {
  // Controller function to get a single user or counselor by ID
  const { id } = req.params; // Extract profile ID from request parameters
  try {
